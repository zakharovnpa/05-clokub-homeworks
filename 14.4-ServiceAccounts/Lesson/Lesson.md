...
#### Выбрать любимый образ контейнера, подключить сервис-акаунты и проверитьдоступность API Kubernetes
...
sh-5.1# 
```
* Прописываем новые переменные среды:
...
curl -H "Authorization: Bearer $TOKEN" --cacert $CACERT $K8S/api/v1/
```
#### Ответ: успешное подключение к API Kubernetes:
